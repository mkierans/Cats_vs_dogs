...
def save_train_image(images, validation):
    ...
    for i, file_path in enumerate(images):
        ...
        if not validation:   
            if 'cat' in file_path:
                cv2.imwrite(file_path.split('input')[0] + 'train256gALL/cat/' + 
                            file_path.split('train/')[1], img)
            if 'dog' in file_path:
                cv2.imwrite(file_path.split('input')[0] + 'train256gALL/dog/' + 
                            file_path.split('train/')[1], img)
        else:
            if 'cat' in file_path:
...
#Write this so that it keeps aspect ratios the same as original picture
